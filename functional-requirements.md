...
- [ ] All travel photos are served by Cloudinary at reasonable sizes when the page is viewed at Laptop L and Mobile L sizes. You should not be using CSS to make huge images look small - you should be serving properly-sized images via Cloudinary at different viewport sizes. This must be accomplished by having `<img>` tags displaying travel photos use `srcset` and `sizes` to create responsive images. See this [MDN reference](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images#how_do_you_create_responsive_images) for guidance, in particular the [Resolution switching: Different sizes](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images#resolution_switching_different_sizes) section. 
